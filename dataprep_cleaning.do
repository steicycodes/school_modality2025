...
//race and ethnicity
		...
		replace race_eth=3 if hisplatino==1
		replace race_eth=4 if white==0 & black==0 & hisplatino==0
...
//-------------------------------------------------------- Labeling time invariant
	...
	label define race_eth 1 "White" 2 "Black" 3 "Hispanic" 4 "Asian/Other"
...
//school type 
	...
	//school type dummies
	gen public = 0
		replace public = 1 if (sl058a==4|sl030 == 1|sl058a == 1|sl030 == 2|sl030 == 3|sl058a == 3|sl079 == 1|sl079 == 2|sl079 == 3)	
	gen private = 0
		replace private = 1 if (sl030 == 4|sl058a == 2|sl079 == 4)
	gen home = 0
		replace home = 1 if (sl030 == 5|sl057a == 2|sl079 == 5)
	gen other_sch = 0 
		replace other_sch = 1 if (sl030 == 6|sl058a == 5|sl058a == 5|sl079 == 6|sl057a==3)
	...
	//labeling 
		...
		label define schooltype 1 "Public/Charter/Magnet/Virtual" 2 "Private" 3 "Homeschooled" 4 "Other/NE"
...
//-------------------------------------------------------- Removing other vars
	...
	keep uasid survey_source final_weight working e_remote c_gender c_race_eth c_married_partner c_education2 c_ageres c_medcond c_trumpvoter c_urbanicity agekid income kids c_mediatrust1 c_institutionaltrust1 percievedrisk schooltype c_trust_source_fox
...
//Public School Exit
	...
	//labeling
		label variable publicexit "Exiting Public School"
...
//Public school exit for homeschooling
	...
	//labeling
		label variable publicexithome "Exiting Public School for Homeschooling"
...
//Public school exit for private
	...
	replace publicexitprivate = 1 if (publicexit == 1 & (schooltype==2))
		...
		//labeling
		label variable publicexitprivate "Exiting Public School for Private School"
		
//Public School Return
	gen publicreturn = 0
	bysort uasid: replace publicreturn = 1 if ((_n > 1) & (schooltype ==1) & (schooltype[_n-1] != 1))	
		//labeling
		label variable publicreturn "Returning to Public School"
...
//-------------------------------------------------------- Phases
	//generating phase var 
	gen phase = .
		replace phase = 1 if survey_source <=274 // april 2020 to dec 2020
		replace phase = 2 if survey_source >=276 & survey_source <=346 // jan to june 2021
		replace phase = 3 if survey_source >=350 & survey_source <=461 // aug 2021 to june 2022
		replace phase = 4 if survey_source >=475 & survey_source <=559 // july 2022 to july 2023
		
	//dropping intermediate waves
	drop if phase == .
